/* $Id: change.c,v 1.3 2006/06/20 16:41:32 tim Exp $ */
...
RCSID ("$Id: change.c,v 1.3 2006/06/20 16:41:32 tim Exp $");
...
static void *ChangeViaPlug (PinTypePtr);
static void *SetViaPlug (PinTypePtr);
static void *ClrViaPlug (PinTypePtr);
static void *ChangePinPlug (ElementTypePtr, PinTypePtr);
static void *SetPinPlug (ElementTypePtr, PinTypePtr);
static void *ClrPinPlug (ElementTypePtr, PinTypePtr);
...
static ObjectFunctionType ChangePlugFunctions = {
  NULL,
  NULL,
  NULL,
  ChangeViaPlug,
...
static ObjectFunctionType SetPlugFunctions = {
  NULL,
  NULL,
  NULL,
  SetViaPlug,
...
static ObjectFunctionType ClrPlugFunctions = {
  NULL,
  NULL,
  NULL,
  ClrViaPlug,
...
/* ---------------------------------------------------------------------------
 * changes the plug on a via
 * returns TRUE if changed
 */
static void *
ChangeViaPlug (PinTypePtr Via)
{
  if (!TEST_FLAG (HOLEFLAG, Via))
    {
      AddObjectToFlagUndoList (VIA_TYPE, Via, Via, Via);
      TOGGLE_FLAG (PLUGFLAG, Via);
...
/* ---------------------------------------------------------------------------
 * changes the plug on a pin 
 * returns TRUE if changed
 */
static void *
ChangePinPlug (ElementTypePtr element, PinTypePtr Pin)
{
  if (!TEST_FLAG (HOLEFLAG, Pin))
    {
      AddObjectToFlagUndoList (VIA_TYPE, element, Pin, Pin);
      TOGGLE_FLAG (PLUGFLAG, Pin);
...
/* ---------------------------------------------------------------------------
 * sets the plug on a via
 * returns TRUE if changed
 */
static void *
SetViaPlug (PinTypePtr Via)
{
  if (!TEST_FLAG (PLUGFLAG, Via) && !TEST_FLAG (HOLEFLAG, Via))
    {
      AddObjectToFlagUndoList (VIA_TYPE, Via, Via, Via);
      SET_FLAG (PLUGFLAG, Via);
      ClearPin (Via, VIA_TYPE, 0);
      return (Via);
    }
  return (NULL);
...
/* ---------------------------------------------------------------------------
 * sets the plug on a pin 
 * returns TRUE if changed
 */
static void *
SetPinPlug (ElementTypePtr element, PinTypePtr Pin)
{
  if (!TEST_FLAG (PLUGFLAG, Pin) && !TEST_FLAG (HOLEFLAG, Pin))
    {
      AddObjectToFlagUndoList (VIA_TYPE, element, Pin, Pin);
      SET_FLAG (PLUGFLAG, Pin);
      ClearPin (Pin, PIN_TYPE, 0);
      return (Pin);
    }
  return (NULL);
...
/* ---------------------------------------------------------------------------
 * clears the plug on a via
 * returns TRUE if changed
 */
static void *
ClrViaPlug (PinTypePtr Via)
{
  if (TEST_FLAG (PLUGFLAG, Via) && !TEST_FLAG (HOLEFLAG, Via))
    {
      AddObjectToFlagUndoList (VIA_TYPE, Via, Via, Via);
      CLEAR_FLAG (PLUGFLAG, Via);
      ClearPin (Via, VIA_TYPE, 0);
      return (Via);
    }
  return (NULL);
...
/* ---------------------------------------------------------------------------
 * clears the plug on a pin 
 * returns TRUE if changed
 */
static void *
ClrPinPlug (ElementTypePtr element, PinTypePtr Pin)
{
  if (TEST_FLAG (PLUGFLAG, Pin) && !TEST_FLAG (HOLEFLAG, Pin))
    {
      AddObjectToFlagUndoList (VIA_TYPE, element, Pin, Pin);
      CLEAR_FLAG (PLUGFLAG, Pin);
      ClearPin (Pin, PIN_TYPE, 0);
      return (Pin);
    }
  return (NULL);
...
{
  BDimension value = (Delta) ? Via->Clearance + Delta : Absolute;
...
{
  BDimension value = (Delta) ? Pin->Clearance + Delta : Absolute;
...
{
  BDimension value = (Delta) ? Pad->Clearance + Delta : Absolute;
...
{
  BDimension value = (Delta) ? Line->Clearance + Delta : Absolute;
...
{
  BDimension value = (Delta) ? Arc->Clearance + Delta : Absolute;
...
/* ----------------------------------------------------------------------
 * changes the plug on all selected and visible pins
 * and/or vias. Returns True if anything has changed
 */
Boolean
ChangeSelectedPlugs (int types)
{
  Boolean change = False;

  change = SelectedOperation (&ChangePlugFunctions, False, types);
  if (change)
    {
      Draw ();
      IncrementUndoSerialNumber ();
...
/* ----------------------------------------------------------------------
 * sets the plugs on all selected and visible pins
 * and/or vias. Returns True if anything has changed
 */
Boolean
SetSelectedPlugs (int types)
{
  Boolean change = False;

  change = SelectedOperation (&SetPlugFunctions, False, types);
  if (change)
    {
      Draw ();
      IncrementUndoSerialNumber ();
...
/* ----------------------------------------------------------------------
 * clears the plugs on all selected and visible pins
 * and/or vias. Returns True if anything has changed
 */
Boolean
ClrSelectedPlugs (int types)
{
  Boolean change = False;

  change = SelectedOperation (&ClrPlugFunctions, False, types);
  if (change)
    {
      Draw ();
      IncrementUndoSerialNumber ();
