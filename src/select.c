/* $Id: select.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: select.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
 ...
 * Calls ListSelObjects() routine which creates list of
 * selected objects pointer database.
 */

extern void ListSelObjects(int , void* , void* );

void
SelectedObjOperation (void)
{
  ...
  VISIBLELINE_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, line))
      {
	ListSelObjects (LINE_TYPE, layer, line);
      }
  }
  ENDALL_LOOP;
  ...
  VISIBLEARC_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, arc))
      {
	ListSelObjects (ARC_TYPE, layer, arc);
      }
  }
  ENDALL_LOOP;
  ...
  ALLTEXT_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, text) && TEXT_IS_VISIBLE (PCB, layer, text))
      {
	ListSelObjects (TEXT_TYPE, layer, text);
      }
  }
  ENDALL_LOOP;
  ...
  VISIBLEPOLYGON_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, polygon))
      {
	ListSelObjects (POLYGON_TYPE, layer, polygon);
      }
  }
  ENDALL_LOOP;
    ...
    ELEMENT_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, element))
      {
	ListSelObjects (ELEMENT_TYPE, NULL, element);
      }
  }
  END_LOOP;
    ...
    ELEMENT_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, &ELEMENT_TEXT (PCB, element)))
      {
	ListSelObjects (ELEMENTNAME_TYPE, NULL, &ELEMENT_TEXT (PCB, element));
      }
  }
  END_LOOP;
    ...
    ELEMENT_LOOP (PCB->Data);
  {
    PIN_LOOP (element);
    {
      if (TEST_FLAG (SELECTEDFLAG, pin))
     {
	ListSelObjects (PIN_TYPE, element, pin);
     }
    }
    END_LOOP;
    ...
    PAD_LOOP (element);
    {
      if (TEST_FLAG (SELECTEDFLAG, pad))
      {
	ListSelObjects (PAD_TYPE, element, pad);
      }
    }
    END_LOOP;
  }
    ...
    VIA_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, via))
      {
	ListSelObjects (VIA_TYPE, NULL, via);
      }
  }
  END_LOOP;
    ...
    RAT_LOOP (PCB->Data);
  {
    if (TEST_FLAG (SELECTEDFLAG, line))
      {
	ListSelObjects (RATLINE_TYPE, NULL, line);
      }
  }
  END_LOOP;
