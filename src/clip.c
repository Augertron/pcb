/* $Id: clip.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
#define TO_SCREEN(a)	((Position)((float)(a) * Local_Zoom))
#define XORIG dxo
#define YORIG dyo
...
RCSID("$Id: clip.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
                 ...
                 int y1, int x2, int y2)
{
  ...
  if (ClipLine (pts)) {
    int tx1, tx2, ty1, ty2, dx, dy;

    tx1 = TO_DRAW_X(pts[0].X);
    ty1 = TO_DRAW_Y(pts[0].Y);
    dx = TO_SCREEN(pts[1].X - pts[0].X);
    dy = TO_SCREEN(pts[1].Y - pts[0].Y);
    tx2 = tx1 + dx;
    ty2 = ty1 + dy;

    /* Use of (dx,dy) solves most problems but when line width	*/
    /* >> line length, zooming out to a critical distance makes	*/
    /* dx = 0 or dy = 0, for which XDrawLine assumes that the	*/
    /* degenerate line is not angled.  Must draw a polygon in	*/
    /* this case.						*/

    XDrawLine (dpy, d, gc, tx1, ty1, tx2, ty2);
  }
}
