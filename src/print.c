/* $Id: print.c,v 1.3 2006/06/20 17:49:05 tim Exp $ */
...
RCSID("$Id: print.c,v 1.3 2006/06/20 17:49:05 tim Exp $");
...
SetPrintColor (Pixel X11Color)
{
  ...
  result = XQueryColor (Dpy,
			DefaultColormapOfScreen (pcbScreen (Output.Toplevel)),
			&rgb);
...
/* ---------------------------------------------------------------------------
 * creates a drill-information file for plugged vias, if the device is able
 * returns != zero on error
 */
...
PrintPlugs (void)
{
  ...
  if (Device->HandlesDrill)
    {
      if (SetupPrintFile
	  (GlobalDOSFlag ? "pvia" : "plugged-via",
	   "plugged via information", FILE_DRILL))
	{
	  return (1);
	}
      SetPrintColor (PCB->PinColor);
	...
	index++;
	for (i = 0; i < drill->PinN; i++)
	  if (TEST_FLAG (PLUGFLAG, drill->Pin[i]))
	    Device->Drill (drill->Pin[i], index);
      }
      END_LOOP;
      /* close the file */
...
PrintBOM (void)
{
  ...
  double pin2x=0.0, pin2y=0.0, pin2angle=0.0;
  int found_pin1;
  int found_pin2;
  int pin_cnt;
  struct passwd *pwentry;
       ...
       Location OffsetX, Location OffsetY, Boolean SilkscreenTextFlag)
{
  ...
  if (PrintLayergroups () || PrintSilkscreen () ||
      PrintDrill () || PrintPlugs() || PrintMask () ||
      PrintPaste () || PrintFab () || PrintBOM () ||
      PrintAssembly () )
    return (1);
  Device->Exit ();
  restoreCursor ();
