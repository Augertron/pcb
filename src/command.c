/* $Id: command.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: command.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
...
#ifdef TCL_PCB
extern Tcl_CmdProc *_tcl_dispatch();
#else
...
static char **argv = NULL;	/* command-line of arguments */
static int argc = 0;
...
/* transfer argc/argv to PCB's global versions and
 * execute the function or action.
 */

int
TclExecutePcbCommand(clientData, interp, in_argc, in_argv)
    ...
    CONST char *in_argv[];
{
   Tk_Window tkwind;
   int i;
   tkwind = (Tk_Window)clientData;

   if (argv != NULL) free(argv);
   argv = (char **)malloc(in_argc * sizeof(char *));

   /* Do we need to use Tcl_Strdup() here? */
   for (argc = 0; argc < in_argc; argc++)
      argv[argc] = (char *)in_argv[argc];
...
void
RegisterUserCommands(Tcl_Interp *interp, Tk_Window tkwind)
{
   int i;
   char command_name[20], *cmdptr;

   sprintf(command_name, "pcb::");
   cmdptr = command_name + 5;
   for (i = 0; i < ENTRIES(Command); i++) {
      sprintf(cmdptr, Command[i].Text);
