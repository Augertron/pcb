/* $Id: lgdialog.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: lgdialog.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
...
LayerGroupDialog (void)
{
  ...
  popup = XtVaCreatePopupShell ("LayerGroups", transientShellWidgetClass,
				Output.Toplevel, XtNallowShellResize, True,
  ...
  last = XtVaCreateManagedWidget ("comment", labelWidgetClass, masterform, 
                                  XtNlabel, "layer-group #", LAYOUT_TOP, NULL);
  ...
  for (layer = 0; layer < MAX_LAYER + 2; layer++)
	...
	default:
      ...
      layernameW = XtVaCreateManagedWidget ("layerName", labelWidgetClass,
					    masterform, XtNfromVert, last,
					    XtNlabel, name, XtNforeground, 
					    color, LAYOUT_TOP, NULL);
      ...
      for (i = 0; i < MAX_LAYER; i++)
	{
	  ...
	  XtVaSetValues (lastInRow, XtNradioGroup, radioGroup[layer],
