/* $Id: draw.c,v 1.2 2006/06/20 16:41:32 tim Exp $ */
...
RCSID("$Id: draw.c,v 1.2 2006/06/20 16:41:32 tim Exp $");
...
LoadBackgroundFile (FILE *f, char *filename)
{
  ...
  display = pcbDisplay (Output.Toplevel);
...
DrawBackgroundImage ()
{
  ...
  if (!bgi || Output.Width != bgi_w || Output.Height != bgi_h)
	...
	XDestroyImage (bgi);
      ...
      bgi = XGetImage (pcbDisplay(Output.Toplevel),
		       DrawingWindow,
  ...
  XPutImage(pcbDisplay(Output.Toplevel), DrawingWindow, Output.fgGC,
	    bgi,
	    0, 0, 0, 0, w, h);
}
...
DrawHole (PinTypePtr Ptr)
{
  ...
  else if (TEST_FLAG (PLUGFLAG, Ptr))
    {
      XSetForeground (Dpy, Output.fgGC, PCB->InvisibleObjectsColor);
      XSetLineAttributes (Dpy, Output.fgGC, TO_SCREEN (Ptr->DrillingHole),
			  LineSolid, CapRound, JoinRound);
      XDrawCLine (Dpy, DrawingWindow, Output.fgGC, Ptr->X,
		  Ptr->Y, Ptr->X, Ptr->Y);
    }
  else
    {
  ...
  if (TEST_FLAG (HOLEFLAG, Ptr))
	...
	XSetForeground (Dpy, Output.fgGC,
			BlackPixelOfScreen (pcbScreen (Output.Output)));
      XSetLineAttributes (Dpy, Output.fgGC, 1, LineSolid, CapRound,
			  JoinRound);
      XDrawCArc (Dpy, DrawingWindow, Output.fgGC, Ptr->X, Ptr->Y,
