/* $Id: main.c,v 1.2 2006/11/05 17:55:34 tim Exp $ */
...
RCSID("$Id: main.c,v 1.2 2006/11/05 17:55:34 tim Exp $");
...
//void CB_Backup (XtPointer, pcbIntervalId *);
void InitDeviceDriver (void);
void InitGC (void);
...
static String Fallback[] = {
  ...
  "Pcb.layerName3:             ground",
  "Pcb.layerName4:             power",
  "Pcb.layerName5:             signal1",
  "Pcb.layerName6:             signal2",
  "Pcb.layerName7:             unused1",
  "Pcb.layerName8:             unused2",
...
static XtResource ToplevelResources[] = {
  ...
  ,
  {"minDrill", "minDrill", XtRInt, sizeof (int),
   XtOffsetOf (SettingType, minDrill), XtRString, "1000"}
  ,
...
#ifdef TCL_PCB
  /* (This is temporary---query the Tk window Output.Output?) */
  Settings.bgColor = WhitePixel(Dpy, DefaultScreen(Dpy));
#else
...
#ifdef TCL_PCB
void
CB_Backup (ClientData unused)
#else
void
CB_Backup (XtPointer ClientData, pcbIntervalId * ID)
#endif
{
  Backup ();

  /* restart timer */
  pcbAddTimeOut (Context, 1000 * Settings.BackupInterval, CB_Backup, NULL);
}
