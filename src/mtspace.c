/* $Id: mtspace.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: mtspace.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
...
{
  int r = mtsb && __box_is_good (&mtsb->box) && mtsb->fixed_count >= 0 && 
    mtsb->even_count >= 0 && mtsb->odd_count >= 0 && mtsb->keepaway > 0 && 1;
  assert (r);
  return r;
}
static int
__mtspace_is_good (mtspace_t * mtspace)
{
  ...
  int r = mtspace && mtspace->rtree && __box_is_good (&mtspace->bounds) && 1;
  assert (r);
  return r;
}
#endif /* !NDEBUG */
