/* $Id: output.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: output.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
 ...
 */
//static void CB_ScrollX (XtPointer, pcbIntervalId *);
//static void CB_ScrollY (XtPointer, pcbIntervalId *);
static void CB_Panner (Widget, XtPointer, XtPointer);

static pcbIntervalId SxID = 0, SyID = 0;
static Location LastX, LastY;

Boolean ActiveDrag (void);
void StopAutoScroll (void);
void DrawClipped (Region);
...
#ifdef TCL_PCB
  width = Tk_Width(Output.Output);
  height = Tk_Height(Output.Output);
#else
  ...
  if (width != Output.Width || height != Output.Height)
    {
      ...
      Offscreen = XCreatePixmap (Dpy, Output.OutputWindow, width, height,
				 DefaultDepthOfScreen (pcbScreen
						       (Output.Output)));
...
#ifdef TCL_PCB
  Tcl_EvalEx(pcbinterp, "catch {UpdatePanner}", -1, 0);
#else
...
#ifdef TCL_PCB
static void
CB_ScrollX (ClientData unused)
#else
static void
CB_ScrollX (XtPointer unused, pcbIntervalId * time)
#endif
{
  SxID = 0;
  HideCrosshair (False);
  ...
  SxID = pcbAddTimeOut (Context, SCROLL_TIME, CB_ScrollX, NULL);
}

#ifdef TCL_PCB
static void
CB_ScrollY (ClientData unused)
#else
static void
CB_ScrollY (XtPointer unused, pcbIntervalId * time)
#endif
{
  SyID = 0;
  HideCrosshair (False);
  ...
  SyID = pcbAddTimeOut (Context, SCROLL_TIME, CB_ScrollY, NULL);
}
...
{
  if (SxID)
    pcbRemoveTimeOut (SxID);
  if (SyID)
    pcbRemoveTimeOut (SyID);
  if (SxID == 0 && SyID == 0)
    RestoreCrosshair (True);
  SxID = 0;
  SyID = 0;
...
void
OutputEvent (ClientData clientData, XEvent *Event)
{

#else

void
OutputEvent (pcbWidget W, XtPointer ClientData, XEvent * Event, Boolean * Flag)
{
      ...
      if (has_entered)
	{
	  ...
	  if (ActiveDrag ())
	      ...
	      if (x < 0 || x >= Output.Width)
		{
		  ...
		  SxID =
		    pcbAddTimeOut (Context, SCROLL_TIME, CB_ScrollX, NULL);
		}
	      if (y < 0 || y >= Output.Height)
		{
		  LastY = SGN (y) * Settings.Grid;
		  SyID =
		    pcbAddTimeOut (Context, SCROLL_TIME, CB_ScrollY, NULL);
		}
	    }
	  else
	    HideCrosshair (True);
...
/* This routine modifies statusline and cursor position in 
 * layout window. In addition it also updates Window menu,
 * enable/disable Printmenu if  PCB contains data and
 * enable/disable Netlist if Netlist data exist.
 */
void
SetOutputLabel (pcbWidget W, char *Text)
{
   char command[256];
   char *windowName = Tk_PathName(W);
   sprintf(command, "%s configure -text \"%s\"\n",  windowName, Text);
   Tcl_Eval(pcbinterp, command);
   sprintf(command, "RefreshMenus %s %s", PCB->Data ?
		(IsDataEmpty(PCB->Data) ? "disabled" : "normal") : "disabled",
		PCB->NetlistLib.MenuN ? "normal" : "disabled");
   Tcl_Eval(pcbinterp, command);
}
