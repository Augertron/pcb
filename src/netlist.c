/* $Id: netlist.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: netlist.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
...
highlight:
   /* Scroll Netlist window and highlight the
   *  currnet net. so this is visible.
    */
    if (EnabledOnly && netmenu) {
       sprintf(string, "HighlightNet {%s}", netmenu->Name);
       Tcl_Eval(pcbinterp, string);
    }
    return netmenu;
}
