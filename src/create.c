/* $Id: create.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $ */
...
RCSID("$Id: create.c,v 1.1.1.1 2006/04/10 22:05:23 tim Exp $");
...
/* Added BY NP 7/22/2005 */
/* ---------------------------------------------------------------------------
 * creates a new polygon from the diagonal pads. Dialognal pads
 * have discrepancy with getber output and connection finding
 * routine. This routine converts diagonal pads to polygon temporary
 * to handle both issue separately.
 * Note : Its user's responsibility to remove polygon once its used
 * to avoid memory leak.
 */
PolygonTypePtr
CreateNewPolygonFromPad (LayerTypePtr Layer,
			       Location X1, Location Y1,
			       Location X2, Location Y2, int thickness,
			       int Flags)
{
  PolygonTypePtr polygon = CreateNewPolygon (Layer, Flags);
  ...
  XMIN  = X1 - width * cos(angle1);
  YMIN  = Y1 - width * sin(angle1);
  XMAX = X2 + width * cos(angle1);
  YMAX = Y2 + width * sin(angle1);
  offsetX = width * cos(angle2);
  offsetY = width * sin(angle2);

  p1.X = (XMAX + offsetX);
  p1.Y = (YMAX + offsetY);
  CreateNewPointInPolygon (polygon,  p1.X, p1.Y);
  p2.X = (XMAX - offsetX);
  p2.Y = (YMAX - offsetY);
  CreateNewPointInPolygon (polygon,  p2.X, p2.Y);
  p3.X = (XMIN - offsetX);
  p3.Y = (YMIN - offsetY);
  CreateNewPointInPolygon (polygon,  p3.X, p3.Y);
  p4.X = (XMIN + offsetX);
  p4.Y = (YMIN + offsetY);
  CreateNewPointInPolygon (polygon,  p4.X, p4.Y);
  SetPolygonBoundingBox (polygon);
  return (polygon);
}
