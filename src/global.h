 ...
 *
 *  RCS: $Id: global.h,v 1.2 2006/11/05 17:55:34 tim Exp $
 */
...
/* The Tcl/Tk version bypasses Xt, so we redefine a number of	*/
/* standard types and routines for either Xt or Tcl/Tk.		*/

#define pcbWidget	Tk_Window
#define pcbDisplay	Tk_Display
#define pcbScreen	Tk_Screen
#define pcbWindow	Tk_WindowId
#define pcbAddTimeOut(a, b, c, d)  Tcl_CreateTimerHandler(b, c, d)
#define pcbRemoveTimeOut	   Tcl_DeleteTimerHandler
#define pcbTimeOutProc		   Tcl_TimerProc *
#define pcbIntervalId		   Tcl_TimerToken

#else /* (!TCL_PCB)---Xt definitions */

#define pcbWidget	Widget
#define pcbDisplay	XtDisplay
#define pcbScreen	XtScreen
#define pcbWindow	XtWindow
#define pcbAddTimeOut	XtAppAddTimeOut
#define pcbRemoveTimeOut XtRemoveTimeOut
#define pcbTimeOutProc	XtTimerCallbackProc
#define pcbIntervalId	XtIntervalId
...
{
  pcbWidget Toplevel,		/* toplevel widget */
    StatusLine,			/* label widget of statusline */
    CursorPosition,		/* current cursor position */
    Control,			/* control widget */
    Menu,			/* popup menu */
    Message,			/* fields for user message */
    MasterForm,			/* the widgets that holds all the others */
  ...
  GC bgGC,			/* background and foreground; */
    ...
    GridGC;			/* for the grid */
  BDimension cw, ch,		/* canvas width/height */
    Width, Height;		/* sizes of output window (porthole) */
  Cursor XCursor;		/* used X cursor */
  unsigned long XCursorShape;	/* and its shape (cursorfont) */
 ...
 */
typedef struct			/* all objects have a Bounding box and ID */
{
  BoxType BoundingBox;
  long int ID;
...
{
  BoxType BoundingBox;
  long int ID,			/* unused when contained in elements */
    Flags;
  BDimension Thickness, Clearance;
  Location Width, Height,	/* length of axis */
    X, Y;			/* center coordinates */
  long int StartAngle, Delta;	/* the two limiting angles in degrees */
} ArcType, *ArcTypePtr;
...
typedef struct			/* holds information about one layer */
{
  ...
  Cardinal LineN,		/* number of lines */
    ...
    ArcN,			/* and arcs */
    LineMax, TextMax, 
    PolygonMax, ArcMax;		/* max number from malloc() */
...
typedef struct
{
  ...
  Location X, Y;		/* center and diameter */
  char *Name, *Number;
  void *Element;
  void *Spare;
...
typedef struct
{
  BoxType BoundingBox;
  long int ID, Flags;
  TextType Name[MAX_ELEMENTNAMES];	/* the elements names; see macro.h */
  					/* description text */
  					/* name on PCB second, value third */
  Location MarkX, MarkY;	/* position mark */
  Cardinal PinN, PinMax, PadN, PadMax, 
  LineN, LineMax, ArcN, ArcMax;	/* number of pins, lines and arcs */
...
typedef struct			/* a single symbol */
{
  ...
  Cardinal LineN,		/* number of lines */
    LineMax;
  BDimension Width, Height,	/* size of cell */
...
typedef struct			/* holds all objects */
{
  Cardinal ViaN,		/* number of vias */
    ...
    RatN,			/* and rat-lines */
    RatMax;			/* max number from malloc() */
  PinTypePtr Via;		/* pointer to object data */
  ElementTypePtr Element;
  RatTypePtr Rat;
  rtree_t *via_tree, *element_tree, *pin_tree, *pad_tree, 
...
typedef struct			/* holds drill information */
{
  ...
  Cardinal ElementN,		/* number of elements using this drill size */
...
/* ---------------------------------------------------------------------------
   most of the information is saved with layout and 
   initialized from resources when a new board design is started
   this struct is also used for the remove list and for buffer handling 
 */
typedef struct			/* holds information about board layout */
{				
  long int ID, Flags;		/* see macro.h */
  ...
  Boolean Changed,		/* layout has been changed */
    ViaOn, ElementOn, RatOn, 
    InvisibleObjectsOn, PinOn,	/* visibility flags */
  ...
  long int CursorX, CursorY,	/* cursor position as saved with layout */
    Clipping;
  BDimension GridOffsetX, GridOffsetY,	/* as saves with layout */
    MaxWidth, MaxHeight;		/* max allowed size */
...
typedef struct			/* information about the paste buffer */
{
  Location X, Y;		/* offset */
  BoxType BoundingBox;
  DataTypePtr Data;		/* data; not all members of PCBType are used */
...
{
  PointType Point1, Point2;	/* start and end position */
  long int State;
  Boolean otherway;
...
{
  Location X, Y;		/* saved position when MOVE_MODE entered */
  BoxType BoundingBox;
...
typedef struct			/* holds cursor information */
{
  ...
  Location X, Y,		/* position in PCB coordinates */
    MinX, MinY, MaxX, MaxY;	/* lowest and highest coordinates */
...
typedef struct			/* some resources... */
{
  ...
  String DefaultLayerName[MAX_LAYER], 
    ...
    FileCommand, ElementCommand, PrintFile, LibraryCommand, 
    LibraryContentsCommand, LibraryTree,	/* path to library tree */
    ...
    FilePath, RatPath, RatCommand, FontPath, ElementPath, LibraryPath, 
  ...
  Location PinoutOffsetX, PinoutOffsetY;	/* offset of origin */
  Position PinoutTextOffsetX, PinoutTextOffsetY;/* offset of text */
...
typedef struct			/* needs and abilities of a driver */
{
  ...
  Boolean MirrorFlag,
    RotateFlag, InvertFlag;	/* several flags */
  BDimension OffsetX, OffsetY;	/* offset from lower/left corner */
  float Scale;			/* scaling */
...
#define	UNDO_REMOVE_POINT		0x0008	/* remove polygon/... points */
#define	UNDO_INSERT_POINT		0x0010	/* insert polygon/... points */
