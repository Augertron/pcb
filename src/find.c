/* $Id: find.c,v 1.5 2006/11/05 17:55:34 tim Exp $ */
...
RCSID("$Id: find.c,v 1.5 2006/11/05 17:55:34 tim Exp $");
...
{
/* Original code -- incompatible with diagonal pads case.
...
*/
  if (TEST_FLAG (SQUAREFLAG, Pad))
  {
	...
	{
		/* square end pads */
		return IsLineInRectangle (MIN ((Pad)->Point1.X, (Pad)->Point2.X) -
								((Pad)->Thickness + 1) / 2,
							    MIN ((Pad)->Point1.Y, (Pad)->Point2.Y) -
								((Pad)->Thickness + 1) / 2,
							    MAX ((Pad)->Point1.X,(Pad)->Point2.X) +
								((Pad)->Thickness + 1) / 2,
							    MAX ((Pad)->Point1.Y,(Pad)->Point2.Y) +
								((Pad)->Thickness + 1) / 2, (Line));
		
	} else {
		/* Diagonal pads. */
		return IsLineInDiagonalPad((Pad), (Line));
	}
  } else {
...
*/
  if (TEST_FLAG (SQUAREFLAG, Pad))
  {
	...
	{
		/* square end pads */
		return IsArcInRectangle (MIN ((Pad)->Point1.X, (Pad)->Point2.X) -
							((Pad)->Thickness + 1) / 2,
							MIN ((Pad)->Point1.Y, (Pad)->Point2.Y) -
							((Pad)->Thickness + 1) / 2,
							MAX ((Pad)->Point1.X, (Pad)->Point2.X) +
							((Pad)->Thickness + 1) / 2,
							MAX ((Pad)->Point1.Y, (Pad)->Point2.Y) +
							((Pad)->Thickness + 1) / 2, (Arc));
	} else {
		/* Diagonal pads. */
		return IsArcInDiagonalPad((Pad), (Arc));
	}
  } else {
...
/* ORIGINAL CODE -- incompatible to handle diagonal pads -- BY NP */
/*
static Boolean
...
/* Modified BY NP 7/25/2005 */
/* Removed bug for diagonal pad to diagonal pad
 * intersection code.
 */
/* Made compatible to handle diagonal pads */
static Boolean
PadPadIntersect (PadTypePtr p1, PadTypePtr p2)
{
   if (TEST_FLAG (SQUAREFLAG, p1) && TEST_FLAG (SQUAREFLAG, p2))
    {
      BoxType b1, b2;
      PolygonTypePtr poly1 = 0, poly2 = 0;
      Boolean isIntersect = False;

      if ((p1->Point1.X == p1->Point2.X) || (p1->Point1.Y == p1->Point2.Y))
      {
        b1.X1 = MIN (p1->Point1.X, p1->Point2.X) - (p1->Thickness + 1) / 2;
        b1.Y1 = MIN (p1->Point1.Y, p1->Point2.Y) - (p1->Thickness + 1) / 2;
        b1.X2 = MAX (p1->Point1.X, p1->Point2.X) + (p1->Thickness + 1) / 2;
        ...
        poly1 = CreateNewPolygonFromPad (CURRENT, p1->Point1.X,
                                   p1->Point1.Y, p1->Point2.X, p1->Point2.Y,
                                   (p1->Thickness + 1), CLEARPOLYFLAG);
        b1 = poly1->BoundingBox;
      }

      if ((p2->Point1.X == p2->Point2.X) || (p2->Point1.Y == p2->Point2.Y))
      {
         b2.X1 =
		MIN (p2->Point1.X,
		p2->Point2.X) - MAX ((p2->Thickness + 1) / 2 + Bloat, 0);
	  b2.Y1 =
		...
		MAX (p2->Point1.Y,
		p2->Point2.Y) + MAX ((p2->Thickness + 1) / 2 + Bloat, 0);
	  isIntersect = BoxBoxIntersection (&b1, &b2);
      } else {
	  poly2 = CreateNewPolygonFromPad (CURRENT, p2->Point1.X,
                                   p2->Point1.Y, p2->Point2.X, p2->Point2.Y,
                                   ((p2->Thickness + 1) + Bloat*2), CLEARPOLYFLAG);
	  isIntersect = IsPolygonInPolygon(poly1, poly2);
      }
      if (poly2)
	  ...
	  for (i = 0; i < PCB->Data->Layer[layer].PolygonN; i++, polygon++)
	    {
	      ...
	      if (((pv->Clearance == 0) || (TEST_FLAGS (Myflag, pv)
 ...
 */
/* Modified BY NP 7/23/3005 */
/* Updated to make able to handle diagonal pads also */
Boolean
  ...
  if (TEST_FLAG (SQUAREFLAG, pad))
    {
          ...
          ((pad)->Point1.Y == (pad)->Point2.Y))
      {
        ...
        PolygonTypePtr padPolygon = CreateNewPolygonFromPad (CURRENT,
                                                           pad->Point1.X, pad->Point1.Y,
                                                           pad->Point2.X, pad->Point2.Y,
                                                           wid, CLEARPOLYFLAG);
         isIntersect = IsPolygonInPolygon(polygon, padPolygon);
         DestroyObject (PCB->Data, POLYGON_TYPE, CURRENT,
	      ...
	      LayerTypePtr layer, PolygonTypePtr polygon)
{
  ...
  switch (type)
    {
    case LINE_TYPE:
      if ((line->Clearance > 0) && line->Clearance <= 2 * Settings.Bloat)
	{
      ...
      break;
    case ARC_TYPE:
      if ((arc->Clearance > 0) && arc->Clearance <= 2 * Settings.Bloat)
	{
	  AddObjectToFlagUndoList (type, ptr1, ptr2, ptr3);
      ...
      break;
    case PAD_TYPE:
      if ((pad->Clearance > 0) && pad->Clearance <= 2 * Settings.Bloat)
	{
	  AddObjectToFlagUndoList (type, ptr1, ptr2, ptr3);
	  SET_FLAG (TheFlag, pad);
      ...
      break;
    case PIN_TYPE:
      if ((pin->Clearance > 0) && pin->Clearance <= 2 * Settings.Bloat)
	{
	  AddObjectToFlagUndoList (type, ptr1, ptr2, ptr3);
	  SET_FLAG (TheFlag, pin);
...
DRCAll (void)
{
  ...
  if (!IsBad)
	...
	if (!TEST_FLAG (HOLEFLAG, pin))
	  {
	    ...
	    if ((!TEST_FLAG (PLUGFLAG, pin) &&
	    	pin->Thickness - pin->DrillingHole < 2 * Settings.minWid) ||
		(TEST_FLAG (PLUGFLAG, pin) &&
		pin->Thickness < Settings.minWid))
	      {
	        AddObjectToFlagUndoList (PIN_TYPE, element, pin, pin);
	        SET_FLAG (TheFlag, pin);
	...
	if (!TEST_FLAG (HOLEFLAG, pin) && pin->DrillingHole < Settings.minDrill)
	  {
	    AddObjectToFlagUndoList (PIN_TYPE, element, pin, pin);
	    SET_FLAG (TheFlag, pin);
	    Message ("Pin drill size is too small\n");
	    DrawPin (pin, 0);
	    drcerr_count++;
	    SetThing (PIN_TYPE, element, pin, pin);
	    GotoError ();
  ...
  if (!IsBad)
	...
	if (!TEST_FLAG (HOLEFLAG, via))
	  {
	    if ((!TEST_FLAG (PLUGFLAG, via) &&
	    	via->Thickness - via->DrillingHole < 2 * Settings.minWid) ||
		(TEST_FLAG (PLUGFLAG, via) &&
		via->Thickness < Settings.minWid))
	      {
	        AddObjectToFlagUndoList (VIA_TYPE, via, via, via);
